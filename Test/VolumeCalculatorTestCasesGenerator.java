...
public class VolumeCalculatorTestCasesGenerator {
    ...
    public static ArrayList<Integer[] > generateBoundaryTestCases() {
        ...
        for (int iVar=0; iVar < NUM_OF_VAR; iVar++) {
            ...
            for (int iType = 0; iType < NUM_OF_BSV; iType++) { // 遍历全部特殊值
                ...
                for (int var = 0; var < NUM_OF_VAR; var++) // 遍历全部变量，为每个变量赋随机生成的普通值
                    testCase[var] = TestUtils.getVCNormalValue(boundaryVals[2 * var], boundaryVals[2 * var + 1]);
        ...
        int nom_len=TestUtils.getVCNormalValue(boundaryVals[0], boundaryVals[1]),
                nom_wth=TestUtils.getVCNormalValue(boundaryVals[2], boundaryVals[3]),
                nom_hgt=TestUtils.getVCNormalValue(boundaryVals[4], boundaryVals[5]);
        // 添加全为普通值的测试样例
    ...
    public static ArrayList<Integer[] > generateWorstCaseTestCases() {
        ...
        for (int iVar=0; iVar < NUM_OF_VAR; iVar++)
            ...
            tmpVals[iVar] = new Integer[]{boundaryVals[2*iVar], boundaryVals[2*iVar]+1, boundaryVals[2*iVar+1]-1, boundaryVals[2*iVar+1], TestUtils.getVCNormalValue(boundaryVals[2*iVar], boundaryVals[2*iVar+1])};
    ...
    public static ArrayList<Integer[] > generateRobustWorstCaseTestCases() {
        ...
        for (int iVar=0; iVar < NUM_OF_VAR; iVar++)
            ...
            tmpVals[iVar] = new Integer[]{boundaryVals[2*iVar]-1, boundaryVals[2*iVar], boundaryVals[2*iVar]+1, boundaryVals[2*iVar+1]-1, boundaryVals[2*iVar+1], boundaryVals[2*iVar+1]+1, TestUtils.getVCNormalValue(boundaryVals[2*iVar], boundaryVals[2*iVar+1])};
    ...
    public static ArrayList<Integer[] > generateStrongNormalEquivalenceClassTestCases() {
        ...
        int sideLen = TestUtils.getVCNormalValue(TestUtils.max(boundaryVals[0], boundaryVals[2], boundaryVals[4]), TestUtils.min(boundaryVals[1], boundaryVals[3], boundaryVals[5]));
        ...
        int notEqualSideId = TestUtils.getVCNormalValue(0, 2),
            equalSideLen = TestUtils.getVCNormalValue(TestUtils.max(boundaryVals[0], boundaryVals[2], boundaryVals[4]), TestUtils.min(boundaryVals[1], boundaryVals[3], boundaryVals[5]));
        // 先给三条边都赋相同的值
        for (int iVar=0; iVar < NUM_OF_VAR; iVar++)
            tmpDate[iVar] = equalSideLen;
        // 给选出的不相等边赋不相等的值
        do
            tmpDate[notEqualSideId] =  TestUtils.getVCNormalValue(boundaryVals[notEqualSideId]*2, boundaryVals[notEqualSideId]*2+1);
        ...
        for (int iVar = 0; iVar < NUM_OF_VAR; iVar++) {
            ...
            do {
                flag = false;
                tmp = TestUtils.getVCNormalValue(boundaryVals[iVar * 2], boundaryVals[iVar * 2 + 1]);
